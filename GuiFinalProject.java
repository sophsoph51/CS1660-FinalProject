		...
		invertedIndices.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Job newjob = dproc.projects().regions().jobs().submit(projectid, "us-central1" , new SubmitJobRequest().setJob(new Job().setPlacement(new JobPlacement(). setClusterName(clusterName))
				     ...
				     //https://stackoverflow.com/questions/38303453/dataproc-client-googleapiclient-method-to-get-list-of-all-jobsrunnng-stopp
	                   job= dproc.projects().regions().jobs().get(projectid, "us-central1", id).execute();
					...
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							Job newjob = dproc.projects().regions().jobs().submit(projectid, "us-central1" , new SubmitJobRequest().setJob(new Job().setPlacement(new JobPlacement(). setClusterName(clusterName))
						     ...
						     String id = newjob.getJobUuid();
			                    Job job = dproc.projects().regions().jobs().get(projectid, "us-central1", id).execute();
